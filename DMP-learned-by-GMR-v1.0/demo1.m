...
nbStates = 10; %Number of states (or primitives)
nbRepros =2;
kP = 100; %Initial stiffness gain
kV = 20; %Damping gain
dt = .01; %Time step
...
Y = Data(accId,:) - [(repmat(xT,1,nbData)-Data(posId,:))*kP - Data(velId,:)*kV];
Mu_F = [inv(H'*H)*H'*Y']';
...
for nb=1:nbRepros
  ...
  for n=1:nbData
    ...
    currF0 = (xT-rWLS(nb).currPos)*kP - rWLS(nb).currVel*kV;
    rWLS(nb).F(:,n) = Mu_F * rWLS(nb).H(:,n);
...
plot(Y(1,:),'-','linewidth',5,'color',[.75 .75 .75]);
for nb=1:nbRepros
